...
if __name__ == '__main__':
    ...
    # 动态推导路径（假设代码在 Emotion_analysis/sentimentAnalysis.py）
    project_root = Path(__file__).resolve().parent  # 直接定位到 Emotion_analysis 目录
    model_path = project_root / "models" / "bert-base-chinese"

    # 调试输出
    print(f"模型绝对路径: {model_path}")
    print(f"模型目录是否存在: {model_path.exists()}")
    print(f"config.json 是否存在: {(model_path / 'config.json').exists()}")

    # 断言检查（替换为实际文件列表）
    required_files = ["config.json", "pytorch_model.bin", "vocab.txt"]
    for file in required_files:
        assert (model_path / file).exists(), f"文件 {file} 缺失！路径: {model_path / file}"
